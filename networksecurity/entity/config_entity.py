...
import os
from networksecurity.constant.training_pipeline.constants import *

class TrainingPipelineConfig:
    def __init__(self,timestamp=datetime.now()):
        # we set up timestamp, so that we later save artifacts under specific date. Data versioning
...
class DataIngestionConfig:
    """
    Configuration class for managing all paths and parameters related to the data ingestion process.

    Attributes:
        data_ingestion_dir (str): Root directory for all data ingestion artifacts.
        feature_store_file_path (str): File path for storing raw or preprocessed features.
        training_file_path (str): Path where the training dataset will be saved.
        testing_file_path (str): Path where the testing dataset will be saved.
        train_test_split_ratio (float): Ratio used to split the data into training and testing sets.
        collection_name (str): Name of the database collection to fetch raw data from.
        database_name (str): Name of the source database for data ingestion.
    """
...
class DataValidationConfig:
    def __init__(self,training_pipeline_config:TrainingPipelineConfig):
        ...
        self.data_drift_report_file_path: str = os.path.join(
            ...
            DATA_VALIDATION_DATA_DRIFT_REPORT_FILE_NAME,
        )
        self.concept_drift_report_file_path: str = os.path.join(
            self.data_validation_dir,
            DATA_VALIDATION_DRIFT_REPORT_DIR,
            DATA_VALIDATION_CONCEPT_DRIFT_REPORT_FILE_NAME,
        )
